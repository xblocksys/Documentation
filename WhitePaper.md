...
이것은 n개의 서로 다른 대상(문서)의 상태를 변화 시키는 각각의 사건 집합을 T<sub>1</sub>, T<sub>2</sub>, T<sub>3</sub>, ... T<sub>n</sub> 이라 할 때, 각가의 사건 집합을 독립적인 선형 구조로 구성하는 것이 가능함을 의미한다. 즉, 동일한 대상을 갖는 특정 사건 집합 내에서만 순차성이 보장된다면, 서로 다른 사건 집합 T<sub>i</sub> 과 T<sub>j</sub> 에 속하는 사건 t<sub>i</sub> 와 t<sub>j</sub> 는 서로에 대하여 순차적으로 처리되어야 할 필요도, 동일한 직렬화 구조에 포함될 필요도 없다.
